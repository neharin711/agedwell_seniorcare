...
import 'package:flutter_pro/vol_view_request.dart'; // Import Lottie package

void main() {
  runApp(vol_view_req_cat());
}

class vol_view_req_cat extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      ...
      home: vol_view_req_cat_Pages(),
    );
  }
}

class vol_view_req_cat_Pages extends StatefulWidget {

  @override
  _vol_view_req_cat_State createState() => _vol_view_req_cat_State();
}

class _vol_view_req_cat_State extends State<vol_view_req_cat_Pages> {
  ...
  Future<void> _loadMessages() async {
    try {
      ...
      String categoryUrl = ip + "/api/view_vol_req_cat";
            ...
            child: ListView.builder(
              ...
              itemBuilder: (context, index) {
                return JobCardrequest(
                  item: "Requirements: ${messageData[index]['v_cat'] ?? 'Item Not Available'}",

                  onTap: () {
                    Navigator.push(
                      context,
                      MaterialPageRoute(
                        builder: (context) => VolViewReqPage(
                          d_cat_id: messageData[index]['v_cat_id'].toString(),
                        ),
                      ),
                    );
                  },
                    ...
                    showDialog(
                      ...
                      builder: (BuildContext context) {
                        return DonationPopup(
                          ...
                          item_id: " ${messageData[index]['v_req_id'] ?? 'Item Not Available'}",
                        );
                      },
                    );
                  },
...
class JobCardrequest1 extends StatelessWidget {
  final String item;
  final String dname;
  final String status;
  final VoidCallback onTap;

  const JobCardrequest1({
    Key? key,
    required this.item,
    required this.dname,
