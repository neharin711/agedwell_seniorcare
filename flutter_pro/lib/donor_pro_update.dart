...
class DProUpdate extends StatefulWidget {
  static const String routeName = '/IndividualAccountUpdate';
  const DProUpdate({Key? key}) : super(key: key);

  @override
  State<DProUpdate> createState() => _VProUpdateState();
}

class _VProUpdateState extends State<DProUpdate> {
  ...
  Future<void> _loadMessages() async {
    try {
      ...
      String categoryUrl = '$ip/api/view_donor_update';
      ...
      if (status == "true") {
        setState(() {
          ...
          if (messageData.isNotEmpty) {
            ...
            controllerList[0].text = userData['dname'] ?? '';
            controllerList[1].text = userData['demail'] ?? '';
            controllerList[2].text = userData['dphone'] ?? '';
            controllerList[3].text = userData['doccupation'] ?? '';
            controllerList[4].text = userData['ddateofbirth'] ?? '';
            controllerList[5].text = userData['dcity'] ?? '';
            controllerList[6].text = userData['username'] ?? '';
            controllerList[7].text = userData['password'] ?? '';
            userGender = userData['dgender'];
            dob = userData['ddateofbirth'];
            imgUrl = '$ip/${userData['dp_image']}';
          }
        });
      } else {
  ...
  Future<void> _submitForm(BuildContext context) async {
    ...
    String updateUrl = '$url/api/donor_update';
