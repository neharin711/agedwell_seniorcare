...
import 'd_home.dart'; // Import Lottie package

void main() {
  runApp(donor_view_req());
}

class donor_view_req extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      ...
      home: donor_view_reqPage(d_cat_id: '',),
    );
  }
}

class donor_view_reqPage extends StatefulWidget {
  final String d_cat_id; // Declare this variable

  donor_view_reqPage({required this.d_cat_id});
  @override
  _donor_view_reqState createState() => _donor_view_reqState();
}

class _donor_view_reqState extends State<donor_view_reqPage> {
  ...
  Future<void> _loadMessages() async {
    try {
      ...
      String categoryUrl = ip + "/api/view_donor_req";
...
class _DonationPopupState extends State<DonationPopup> {
  ...
  Future<void> _submitDonation() async {
    try {
      ...
      String apiUrl = ip + "/api/add_donor_req"; //
      final pref = await SharedPreferences.getInstance();
      String lid = pref.getString("lid") ?? "";
  ...
  Widget build(BuildContext context) {
    return AlertDialog(
      title: Text('Are you ready to donate this?'),
      content: Column(
      ...
      actions: <Widget>[
        TextButton(
          onPressed: () {
            if (_selectedOption == 'Pickup' && _addressController.text.isEmpty) {
              // Show an error if address is required but not entered
              ScaffoldMessenger.of(context).showSnackBar(
